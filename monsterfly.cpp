#include "MonsterFly.h"
#include "jugador.h"

#include <QTransform>
#include <QGraphicsScene>
#include <QtMath>
#include <QDebug>

/* ---------- constantes ---------- */
namespace {
const qreal SCALE          = 0.4;
const float ATK_RANGE      = 60.f;
const float DET_RANGE      = 250.f;
const float Y_TOLERANCE    = 12.f;
const float BOB_AMPLITUDE  = 10.f;
const float BOB_SPEED      = 2.5f;     // Hz
}

/* helper para cargar “000 … 017” */
static Animacion loadSet(const QString& patt,int frames,bool mirror=false)
{
    Animacion a; a.fps = 10.f;
    QTransform flip; flip.scale(-1,1);

    for(int i=0;i<frames;++i){
        QString file = patt.arg(i,3,10,QChar('0'));
        QPixmap p(file);
        if(p.isNull()){
            qWarning()<<"[MonsterFly] Falta"<<file; continue;
        }
        p = p.scaled(p.size()*SCALE, Qt::KeepAspectRatio,
                     Qt::SmoothTransformation);
        if(mirror) p = p.transformed(flip);
        a.frames.append(p);
    }
    return a;
}

/* ---------- ctor ---------- */
MonsterFly::MonsterFly(QObject* parent):Enemigo(parent)
{
    addAnim(Estado::Idle  , loadSet(":/resources/0_Monster_Idle_%1.png"  ,18));
    addAnim(Estado::Walk  , loadSet(":/resources/0_Monster_Fly_%1.png"   ,18));
    addAnim(Estado::Attack, loadSet(":/resources/0_Monster_Attack_%1.png",18));
    addAnim(Estado::Hurt  , loadSet(":/resources/0_Monster_Fall_%1.png"  ,18));
    addAnim(Estado::Death , loadSet(":/resources/0_Monster_Dying_%1.png" ,18));

    if(!animActual().frames.isEmpty()){
        QPixmap f = animActual().frames.first();
        setOffset(-f.width()/2, -f.height()/2);
        setTransformOriginPoint(f.width()/2, f.height()/2);
    }
}

/* ---------- colisiones ---------- */
QRectF MonsterFly::boundingRect()const{
    const QPixmap& p = pixmap();
    return {-p.width()/2, -p.height()/2, p.width(), p.height()};
}
QPainterPath MonsterFly::shape()const{
    QPainterPath s; s.addRect(boundingRect()); return s;
}

/* ---------- IA ---------- */
void MonsterFly::updateAI(float dt)
{
    if(!target()) return;

    float dx = target()->transform().getPosition().x() - pos().x();

    float selfFeet = sceneBoundingRect().bottom();
    float targFeet = selfFeet;
    if(auto* j = dynamic_cast<Jugador*>(target()))
        if(auto* gi = j->graphicsItem())
            targFeet = gi->sceneBoundingRect().bottom();

    /* ataque */
    if(qAbs(dx)<ATK_RANGE && qAbs(targFeet-selfFeet)<Y_TOLERANCE){
        setEstado(Estado::Attack); m_velX=0;
        m_faceRight = dx>=0; return;
    }
    /* persecución */
    if(qAbs(dx)<DET_RANGE){
        setEstado(Estado::Walk);
        m_velX = (dx>0?1:-1)*120.f;
        m_faceRight = dx>=0; return;
    }
    /* patrulla */
    setEstado(Estado::Walk);
    m_patrolTime+=dt;
    if(m_patrolTime>2.2f){ m_patrolDir=-m_patrolDir; m_patrolTime=0; }
    m_velX = m_patrolDir*80.f;
    m_faceRight = m_patrolDir>0;
}

/* ---------- daño / muerte ---------- */
void MonsterFly::takeDamage(int dmg)
{
    if(isDead()) return;
    Enemigo::takeDamage(dmg);
    if(isDead()) startDeath();
}
void MonsterFly::startDeath()
{
    m_deadAnim = true; m_deadTimer=0.f;
    setEstado(Estado::Death);
    auto& a=animActual(); a.idx=0; a.acum=0;
}

/* ---------- update ---------- */
void MonsterFly::update(float dt)
{
    if(!m_deadAnim) updateAI(dt);     /* IA */

    /* bobbing vertical */
    m_bobTime += dt * BOB_SPEED * 2*M_PI;
    float dy = std::sin(m_bobTime)*BOB_AMPLITUDE;
    moveBy(m_velX*dt, dy - m_prevBob);
    m_prevBob = dy;

    /* animación */
    auto& a = animActual(); a.avanzar(dt);
    if(m_deadAnim && a.idx==a.frames.size()-1) a.acum=0;

    QPixmap frame = a.actual();
    if(m_faceRight) frame = frame.transformed(
            QTransform().scale(-1,1));
    setPixmap(frame);
    setOffset(-frame.width()/2, -frame.height()/2);

    if(m_deadAnim){
        m_deadTimer += dt;
        if(m_deadTimer > 1.f/a.fps &&
            a.idx == a.frames.size()-1)
            setVisible(false);
    }
}
