    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(pantallaInicio, &PantallaInicio::iniciarJuegoPresionado, this, [=]() {
        // Mostrar video introductorio
        auto* intro = new VideoIntro(this);
        intro->setVideo("resources/historia.mp4");

        connect(intro, &VideoIntro::videoTerminado, this, [=]() {
            if (!menuOpciones) {
        ...
        mostrarPantalla(intro);  // ← Mostrar el video al presionar "Iniciar juego"
    });
}
...
{
    if (nombre == "Tutorial") {
        auto* videoIntro = new IntroVideoPlayer("resources/templo_olvido_vid.mp4", [=]() {
